% Function to animation the random waypoint mobility model
function AnimationHelper(s_mobility,s_input,time_step)
    
    v_t = 0.1:time_step:s_input.SIMULATION_TIME;
    ...
    hold on;
    grid on;
    %Atribuicao das posicoes x e y das 90 femtocells, geradas aleatoriamente para o cenario
   
    for nodeIndex = 1:s_mobility.NB_NODES
          vh_node_pos(nodeIndex) = plot(vs_node(nodeIndex).v_x(1),vs_node(nodeIndex).v_y(1),'o','color',[0.3 0.3 1]); % plota apenas os nós
    end
    ...
    axis([min(vs_node(1).v_x) max(vs_node(1).v_x) min(vs_node(1).v_y) max(vs_node(1).v_y)]); % Linha que plota a figura principal
     xlim manual; % atribuicao de limites manuais fixos para o mapa no eixo x
    ylim manual;  % atribuicao de limites manuais fixos para o mapa no eixo y
    xlim([0 1000]);
    ylim([0 1000]);
    hold off;
